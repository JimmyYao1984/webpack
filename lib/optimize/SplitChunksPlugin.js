	...
	static normalizeName(option) {
		if(option === true) {
			const fn = (module, chunks, cacheGroup) => {
				...
				// Filename length can't exceed 255 characters otherwise an
				// ENAMETOOLONG error is raised. If the generated name if too
				// long, it is truncated and a unique ID is append. The limit
				// has been set to 200 to prevent `[name].[chunkhash].[ext]`
				// from generating a 256+ character string.
				if(name.length > 200) {
					name = name.slice(0, 200) + "~" + (++guardID);
				}
				return name;
			};
			return fn;
